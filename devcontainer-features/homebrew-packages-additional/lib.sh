...
identifyNonRootUser() {
    ...
    if [ "${requested_user}" != "" ] && [ "${requested_user}" != "auto" ] && [ "${requested_user}" != "automatic" ]; then
        if [ "${requested_user}" = "none" ] || [ "${requested_user}" = "root" ]; then
            err "Homebrew requires a named non-root passwd user. Create one in your image or add the 'nonroot-user' feature before using homebrew-packages-additional."
            exit 1
        fi
        ...
        if isUnsupportedHomebrewUid "${candidate_uid}"; then
            err "Requested user '${requested_user}' resolves to unsupported UID 65532. Create a real named non-root passwd user before using homebrew-packages-additional."
            exit 1
        fi
...
runAsUser() {
    ...
    if commandExists su; then
        ...
        su -s /bin/sh -c "env HOME=$(printf '%q' "${home_dir}") USER=$(printf '%q' "${username}") LOGNAME=$(printf '%q' "${username}") PATH=$(printf '%q' "${PATH}")${escaped_command}" "${username}"
        return
    fi

    err "Unable to switch user. Install setpriv, runuser, or su."
    exit 1
}

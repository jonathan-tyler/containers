...
- When you edit anything under `devcontainer-features/src/<feature>/`, decide the semver bump intentionally and run `./scripts/bump-feature-version.sh` instead of hand-editing `devcontainer-feature.json`.
- `AGENTS.md` is guidance only. The repo hook and CI enforce the bump rule, and `just hooks` configures the local repository to use `.githooks/`.

...
resolve_username() {
    ...
    if [ "${candidate}" = "auto" ] || [ "${candidate}" = "automatic" ] || [ "${candidate}" = "root" ]; then
        ...
        for current_user in \
            ...
            devcontainer \
            nonroot \
            "$(get_username_for_uid 1000)"; do
            if [ -n "${current_user}" ] && { id -u "${current_user}" >/dev/null 2>&1 || is_numeric_uid "${current_user}"; }; then
...
install_packages_if_needed() {
    ...
    if ! command_exists dnf; then
        err "This feature requires dnf when Podman or its rootless helpers are not already present."
        exit 1
    fi

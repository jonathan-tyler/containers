...
identifyNonRootUser() {
    ...
    if [ "${requested_user}" != "" ] && [ "${requested_user}" != "auto" ] && [ "${requested_user}" != "automatic" ]; then
        if [ "${requested_user}" = "none" ] || [ "${requested_user}" = "root" ]; then
            err "Homebrew requires a named non-root passwd user. Create one in your image or add the 'nonroot-user' feature before using homebrew-install."
            exit 1
        fi
        ...
        if isUnsupportedHomebrewUid "${candidate_uid}"; then
            err "Requested user '${requested_user}' resolves to unsupported UID 65532. Create a real named non-root passwd user before using homebrew-install."
            exit 1
        fi

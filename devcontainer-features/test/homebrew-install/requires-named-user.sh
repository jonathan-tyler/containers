...
feature_dir="${repo_root}/src/homebrew-install"
...
if docker run --rm \
    ...
    echo "Expected homebrew-install to reject numeric-only requested users." >&2
    exit 1
fi
...
if docker run --rm \
    ...
    echo "Expected homebrew-install to reject automatic selection from UID 65532." >&2
    exit 1
fi

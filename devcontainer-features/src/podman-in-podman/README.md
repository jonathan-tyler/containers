...
- The feature prefers a real named non-root passwd user for rootless subuid and subgid configuration. If your image only provides `root`, the feature falls back to `root`. Numeric-only runtime users are not used as the target user; add the repo's `nonroot-user` feature if you want a named user without maintaining one in the image.

...
This example runs rootless Podman inside an unprivileged Dev Container launched by rootless Podman.

## What this example proves

Host-side `just test-podman` performs the complete test:

1. It verifies the tested host identity, architecture, subordinate-ID ranges,
   rootless Podman, Just, and the Dev Container CLI.
...
This is a verification-host example, not a portable nested-container product.
It was tested with:

- Linux `x86_64` on a WSL2 kernel;
- host UID `1000` and primary GID `1001`;
- host subordinate UID and GID range `100000:65536`;
...
of rootless Podman. See “Host UID/GID portability” before adapting them.
...
The outer container uses Podman's `keep-id` user namespace. On the tested host,
the outer maps were:

```text
UID: outer 0..999 -> parent 1..1000
UID: outer 1000   -> host 1000
UID: outer 1001..65536 -> parent 1001..65536

GID: outer 0..1000 -> parent 1..1001
GID: outer 1001    -> host 1001
...
These are IDs visible in the **outer** namespace. Do not copy the host's
`100000:65536` range into the image. Linux requires every child mapping target
to be mapped by its parent namespace.
...
- the host user has at least 65,536 subordinate UIDs and GIDs;
...
The fixed `keep-id` arguments assume host UID `1000` and GID `1001`. If your
host differs, update the following as one reviewed change:

- `devcontainer.json` `--userns=keep-id` values;
- the image user's UID and GID;
- `/etc/subuid` and `/etc/subgid` ranges;
- runtime and storage directory ownership; and
- host and nested map assertions in the test scripts.

Do not merely enable `updateRemoteUserUID`; nested subordinate ranges must be
re-derived around the resulting user IDs.

### Host UID/GID portability

Podman can potentially map an arbitrarily numbered non-root host user to fixed
container-side UID `1000` and GID `1001` with:

```text
--userns=keep-id:uid=1000,gid=1001
```

That would preserve predictable image ownership and inner subordinate ranges
without requiring the host user itself to be `1000:1001`. To adopt that model:

1. Remove the exact host UID/GID guard from `test-podman-host.sh` while still
   rejecting host UID `0`.
2. Accept a host subordinate UID/GID range of at least 65,536 IDs regardless
   of its starting number.
3. Keep the image's container-side user and directory ownership fixed.
4. Validate the observed outer and nested maps instead of assuming a mapping
   from host-level subordinate-ID numbers.
5. Test on at least one host whose UID and GID differ from `1000:1001`.

This portability path has not been verified by the example. Treat it as a
documented adaptation strategy rather than a supported configuration.
...
example. A run records host usernames, IDs, kernel details, local paths,
container IDs, image identities, and repository revisions. Review and redact
those files before sharing or promoting them outside your environment.
